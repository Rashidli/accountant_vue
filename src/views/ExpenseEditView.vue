<template>
    <div class="about">
      <div class="container">
        ...
        <form @submit.prevent="updateData" method="post">
          ...
          <div v-if="expense_show.productItems && expense_show.productItems.length">
            <table>
              <tr>
                <th>Məhsul</th>
                <th>Ölçü vahidi</th>
              ...
              <tr v-for="(product, i) in expense_show.productItems" :key="i">
                <td>
                  <select
                    :value="product.name"
  ...
  export default {
    ...
    methods: {
      ...
      showData() {
        axios
          .get("https://sheet.washing.az/api/expense_show/" + this.$route.params.id)
          .then((response) => {
            ...
            this.expense_show.productItems = response.data.expense_products;
            this.totalItemsAmount = response.data.total_price;
            this.expense_show.total_price = response.data.total_price;
          });
      },
      updateData() {
        this.expense_show.total_price = this.totalItemsAmount;
        axios
          .post(
            "https://sheet.washing.az/api/expense_update/" + this.$route.params.id,
            this.expense_show
          )
          .then((response) => {
      ...
      addNewProduct() {
        this.itemName &&
          ...
          this.itemQuantity &&
          this.itemPrice &&
          this.expense_show.productItems.push({
            name: this.itemName,
      ...
      updateTotalAmount() {
        ...
        this.expense_show.productItems.forEach((item) => {
          this.totalItemsAmount = this.totalItemsAmount + item.amount;
        });
      },
  
      removeProductAt(productIndex) {
        this.expense_show.productItems = this.expense_show.productItems.filter(
          (p, i) => {
